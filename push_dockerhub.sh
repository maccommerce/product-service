...
docker build -t maccommerce/product-service:latest -t maccommerce/product-service:"$PROJECT_VERSION" .

echo "${BLUE_COLOR}${BOLD}Pushing docker image to Docker Hub...${NO_COLOR}${NORMAL}"
docker push maccommerce/product-service:latest
docker push maccommerce/product-service:"$PROJECT_VERSION"
